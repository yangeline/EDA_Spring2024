...
> Answer: The results appear in the console, and if it's a package that consists multiple packages in it (such as "tidyverse"), it shows all the packeges that are successfully installed and those that are conlicting (if any).
...
> ANSWER: The README file is in the Metadata folder uploaded to the course page, under the Week 3 module. The README file consists information such as the name, source, date and details for the database and dataset, which is useful for others to retreive and reproduce. The file nameing also clearly demonstrates the necessary information, such as the database name, specific stie number, and the file format.
...
> Answer: Missing data points are marked as NA (which stands for ‘Not Available’). 
...
> Answer: When the NAs are at the beginning at historical data, it is favorable to remove them; when the NAs are at the middle of the data, it is favorable to retain or replace them.
