...
pass_fail <- function(x){
  test_result <- ifelse(x>49, TRUE, FALSE)
  print(test_result)
}
pass_fail
```
...
```{r basics 3}
#if else method
pass_fail_result <- pass_fail(test_score)
pass_fail_result
```

12. QUESTION: Which option of `if` and `else` vs. `ifelse` worked? Why?

> Answer: The 'ifelse' method worked and the 'if' and 'else' didn't. Becaues the 'if' and 'else' function isn't able to take on vector values.   