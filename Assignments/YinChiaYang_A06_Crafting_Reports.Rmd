...
```{r data.summary, message=FALSE, include=FALSE}
...
```{r date labels, echo=FALSE, warning=FALSE}
library(ggplot2)
library(lubridate)
epa_data$month_factor <- factor(
  epa_data$Month,
  levels = 1:12,
  labels = month.abb)
```

```{r boxplot ozone 2018, echo=FALSE, warning=FALSE, fig.cap= "2018 Ozone values by month", fig.dim=c(6,4)}
EPA_boxpolt_ozone_2018 <- epa_data %>% 
  filter(Year == 2018) %>% 
  ggplot(
    mapping = aes(x= month_factor, y= Ozone)) +
    labs(x= "Month", y= "Concentration (µg/m3)", 
         title= "Monthly Ozone Concentrations - 2018") +
  geom_boxplot()
print(EPA_boxpolt_ozone_2018)
```

```{r boxplot ozone 2019, echo=FALSE, warning=FALSE, fig.cap= "2019 Ozone values by month", fig.dim= c(6,4)}
EPA_boxpolt_ozone_2019 <- epa_data %>% 
  filter(Year == 2019) %>% 
  ggplot(
    mapping = aes(x= month_factor, y= Ozone)) +
    labs(x= "Month", y= "Concentration (µg/m3)", 
...
## Git Repository
https://github.com/yangeline/EDA_Spring2024.git
