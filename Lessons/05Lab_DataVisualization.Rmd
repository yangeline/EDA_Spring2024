...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    x=0,
    y=0.025,
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6')) #add text/symbols to the plot
    )
...
PeterPaul.chem.nutrients$month_factor <- factor(
...
#Apply a theme
surface_temp_1999_plot + 
  theme_classic() #theme changes the color under

#Apply a ggtheme
surface_temp_1999_plot + 
  theme_wsj() +  #wsj = wallstreetjournal
  scale_color_wsj()

#Add a title
surface_temp_1999_plot + 
  ggtitle("Surface temperature, 1999") + #add title
  labs(color='Lake') + #add label name
  theme_wsj() #use the settings in wsj
```
...
surface_temp_by_year <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
 ggplot(
    mapping = aes(
      x=daynum,
...
surface_temp_by_date <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
 ggplot(
    mapping = aes(
      x=sampledate,
