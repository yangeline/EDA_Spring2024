...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% #separate by " "(space)
  mutate( Date = my(paste0(Month,"-",Year))) %>% #paste0, 0=there's no separator
...
BR_complete <- inner_join(Rainfall_BR_processed,Temp_BR_processed) #inner_join() to identify if there are N/As, if 100% sure the columns are the same -> cbind()
...
Rainfall_time <- BR_complete %>% 
  ggplot(aes(x=Date, y=Rainfall)) +
  geom_line(alpha=0.5) +
  geom_smooth(method = lm) +
  labs(x= "Time", y= "Rainfall(ml)")
print(Rainfall_time)

Temperature_time <- BR_complete %>% 
...
f_month <- month(first(BR_complete$Date)) #extract month from the first data **first observation need to be on the first row of the data frame
f_year <- year(first(BR_complete$Date)) #extract year from the first data **first observation need to be on the first row of the data frame
...
rainfall_decomp <- stl(BR_rainfall_ts,s.window = "periodic")
plot(rainfall_decomp)

temperature_decomp <- stl(BR_temp_ts,s.window = "periodic")
...
BR_rainfall_components <- as.data.frame(rainfall_decomp$time.series[,1:3]) %>% 
  mutate(Observed = BR_complete$Rainfall, Date = BR_complete$Date)

BR_temperature_components <- as.data.frame(temperature_decomp$time.series[,1:3]) %>% 
  mutate(Observed = BR_complete$Temperature_C, Date = BR_complete$Date)



# Exercise 10: Visualize how the trend maps onto the data for both series

rainfall_trend_plot <- BR_rainfall_components %>% 
  ggplot(aes(x=Date)) +
  geom_line(aes(y=Observed)) +
  geom_line(aes(y=trend), color = "orange") +
  labs(x= "Time", y="Rainfall")
print(rainfall_trend_plot)

temperature_trend_plot <- BR_temperature_components %>% 
  ggplot(aes(x=Date)) +
  geom_line(aes(y=Observed)) +
  geom_line(aes(y=trend), color = "orange") +
  labs(x= "Time", y="Temperature (°C)")
print(temperature_trend_plot)
...
rainfall_trend <- Kendall::SeasonalMannKendall(BR_rainfall_ts)
summary(rainfall_trend)
print(rainfall_trend)

temperature_trend <- Kendall::SeasonalMannKendall(BR_temp_ts)
